        ...
        {cartItems.length === 0 ? <div className="text-center py-8">
            ...
            <div className="divide-y">
              {cartItems.map(item => <div key={item.id} className="py-4 flex items-center">
                  <Link to={`/product/${item.id}`} className="flex-shrink-0 w-20 h-20">
                    <img src={item.images && item.images.length > 0 ? item.images[0] : '/placeholder.svg'} alt={item.name} className="w-full h-full object-cover rounded-md" />
                  </Link>
                  <div className="ml-4 flex-grow">
                    ...
                    <div className="mt-1">
                      {item.salePrice ? <div className="flex items-center">
                        ...
                        </div> : <span className="font-bold">₹{item.price.toFixed(2)}</span>}
                    </div>
                    <div className="mt-2 flex items-center justify-between">
                      <div className="flex items-center border rounded-md">
                        <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="p-1 text-gray-600 hover:text-brand-blue">
            ...
            <div className="mt-6">
              <div className="flex items-center space-x-2">
                <Input placeholder="Enter coupon code" value={couponCode} onChange={e => setCouponCode(e.target.value)} className="flex-grow" />
                <Button onClick={handleCouponApply} disabled={!couponCode || isApplyingCoupon} variant="outline">
              ...
              <Button onClick={handleCheckout} className="w-full mt-4 bg-brand-blue hover:bg-brand-darkBlue">
                Proceed to Checkout
              </Button>
            </div>
          </>}
